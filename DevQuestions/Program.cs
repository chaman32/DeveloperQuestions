...
namespace DevQuestions
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            List<Person> p = personRepo.GetAll().ToList();

            p = p.Where(somebody => somebody.Age >= 18 & somebody.Age < 99).ToList();

            p = p.Where(somebody => somebody.Location == "UK").ToList();

            p = p.Where(somebody => somebody.FirstName.StartsWith("L")).ToList();
